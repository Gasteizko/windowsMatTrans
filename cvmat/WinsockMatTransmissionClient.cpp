...
int WinsockMatTransmissionClient::socketConnect(const char* IP, int PORT)
{
	...
	err = (sockClient = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP));
	if (err < 0) {
...
int WinsockMatTransmissionClient::transmit(cv::Mat image)
{
	...
	base64.Encode(str, &base64encoded);
	
	int full_pack = base64encoded.size() / BUFFER_SIZE;
	int half_pack = base64encoded.size() % BUFFER_SIZE;
	std::cout << base64encoded.size() << std::endl;
	//std::cout << base64encoded << std::endl;
	for (int k = 0; k < full_pack; k++)
	{
		int num1 = BUFFER_SIZE * k;
		for (int i = 0; i < BUFFER_SIZE; i++)
		{
			data.buf[i] = base64encoded[num1+i];
		}
		data.flag = BUFFER_SIZE;
	...
	{
		data.buf[i] = base64encoded[BUFFER_SIZE*full_pack + i];
	}
	data.flag = half_pack;
	if (send(sockClient, (char *)(&data), sizeof(data), 0) < 0)
	{
